...
#' @param distribution (Optional) Specifies the distribution to fit for the linear mixed effect model. [Default] = "LMM", or the lmer() function from lme4. Other options include "LME.Z" = NBZIMM::lme.zig() , "GLMM.Z" = NBZIMM::glmm.zinb(), or "GLMM.NB" = NBZIMM::glmm.nb(). Considering CSS normalizing your data before using the GLMM.Z or the GLMM.NB models. For both, the offset term is set to log(sum(model_data$asvcounts)) for each model.
...
#' @export
#'
LIMON_DistrFit <- function (Obj, Time, Subject, Covariates, model, distribution = "LMM",
                             correlation = NULL)
{
  library(NBZIMM)
  library(lme4)
  ...
  # Check if covariates are numeric or binary
  for (covariate in Covariates) {
    covariate_data <- Obj$SampleData[[covariate]]
    if (!is.numeric(covariate_data) && !all(unique(covariate_data) %in% c(0, 1))) {
      stop(paste("Error: The covariate", covariate, "is not numeric or binary. Please ensure all covariates are numeric or binary."))
    }
  }
  ...
  for (col_name in colnames(Obj$Counts)) {
    ...
    for (covariate in Covariates) {
      ...
      if (length(beta) == 1 && !is.na(beta)) {
        corrected_counts <- corrected_counts - beta*model_frame[[covariate]]
      }
    }
  ...
  # Editing pull the corrected counts here
