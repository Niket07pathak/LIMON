...
#'
#' Uses SPIEC-EASI to fit a network per Time point and return that network object, the optimal covariance or beta matrices per time, and differences in covariance or beta matrices per time. See the \pkg{SpiecEasi} documentation for more details.
#'
...
#'
#' @return Returns a LIMON object with SpiecEasi networks and Covariance or Beta matrices by time added to it. SpiecEasi_Time_i: SpiecEasi object per time point. CovMatrix_Time_i: Covariance or Beta matrix per time point extracted from Spiec Easi. CovMatrix_Diff_ji: Difference in matrices between time j and time i.
#' @export
#'
LIMON_NetInf_Time <- function(Obj, method = "glasso",
                              sel.criterion = "stars",
                              lambda.min.ratio,
                              nlambda ,
                              pulsar.select = TRUE,
  ...
  for (i in seq_along(Obj[["Corrected_Counts_Time"]])) {
    ...
    # Choose the appropriate function based on the method
    if (method == "glasso") {
      matrix <- as.matrix(getOptCov(net_data))
    } else if (method == "mb") {
      matrix <- as.matrix(getOptBeta(net_data))
    } else {
      stop("Invalid method specified. Use 'glasso' or 'mb'.")
    }
